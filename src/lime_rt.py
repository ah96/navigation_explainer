...
localCostmapOriginY = 0 
localCostmapResolution = 0
...
def SaveImageDataForLocalPlanner():
    global footprint_tmp, local_plan_tmp, plan_tmp, global_plan_tmp, costmap_info_tmp, amcl_pose_tmp, tf_odom_map_tmp, tf_map_odom_tmp, odom_tmp

    # Saving data to .csv files for C++ node - local navigation planner
    # Save footprint instance to a file
    footprint_tmp = pd.DataFrame(footprint_tmp)#.transpose()
    footprint_tmp.to_csv('~/amar_ws/src/teb_local_planner/src/Data/footprint.csv', index=False, header=False)

    # Save local plan instance to a file
    local_plan_tmp = pd.DataFrame(local_plan_tmp)#.transpose()
    ...
    plan_tmp = pd.DataFrame(plan_tmp)#.transpose()
    ...
    global_plan_tmp = pd.DataFrame(global_plan_tmp)#.transpose()
    global_plan_tmp.to_csv('~/amar_ws/src/teb_local_planner/src/Data/global_plan.csv', index=False, header=False)
...
def classifier_fn(sampled_instance):

    global localCostmapOriginX, localCostmapOriginY, localCostmapResolution, local_plan_x_list, local_plan_y_list
    ...
    for i in range(0, transformed_plan.shape[0]):
        x_temp = int((transformed_plan.iloc[i, 0] - localCostmapOriginX) / localCostmapResolution)
        y_temp = int((transformed_plan.iloc[i, 1] - localCostmapOriginY) / localCostmapResolution)
    ...
    diff_y = 0
    devs = []
    for j in range(0, len(local_plan_x_list)):
        local_diffs = []
        for k in range(0, len(transformed_plan_xs)):
            diff_x = (local_plan_x_list[j] - transformed_plan_xs[k]) ** 2
            diff_y = (local_plan_y_list[j] - transformed_plan_ys[k]) ** 2
    ...
    if determine_dev_type == True:
        ...
        local_plan_gaps = []
        print('LOCAL PLAN LENGTH = ', len(local_plan_x_list))
        diff = 0
        for j in range(0, len(local_plan_x_list) - 1):
            diff = math.sqrt((local_plan_x_list[j]-local_plan_x_list[j+1])**2 + (local_plan_y_list[j]-local_plan_y_list[j+1])**2 )
    ...
    for i in range(0, sample_size):
        ...
        for j in range(0, local_plans_local.shape[0]):
                x_temp = int((local_plans_local.iloc[j, 0] - localCostmapOriginX) / localCostmapResolution)
                y_temp = int((local_plans_local.iloc[j, 1] - localCostmapOriginY) / localCostmapResolution)
...
def sm_only_obstacles(image, img_rgb, x_odom, y_odom, plan_x_list, plan_y_list):
    # show original image
...
def local_costmap_callback(msg):
    ...
    costmap_info_tmp = [localCostmapResolution, msg.info.width, msg.info.height, localCostmapOriginX, localCostmapOriginY, msg.info.origin.orientation.z, msg.info.origin.orientation.w]
    ...
    segments = sm_only_obstacles(image, image_rgb, x_odom_index, y_odom_index, transformed_plan_xs, transformed_plan_ys)

    # my change - to return grayscale to classifier_fn
    fudged_image = image.copy()
    fudged_image[:] = hide_color = 0

    labels=(1,)
    top = labels

    data, labels = data_labels(image, fudged_image, segments,
                                    classifier_fn, num_samples=1000,
                                    batch_size=2048)
...
sub_footprint = rospy.Subscriber("/move_base/local_costmap/footprint", PolygonStamped, footprint_callback)

# Initalize a subscriber to the odometry
sub_odom = rospy.Subscriber("/mobile_base_controller/odom", Odometry, odom_callback)

sub_amcl = rospy.Subscriber("/amcl_pose", PoseWithCovarianceStamped, amcl_callback)
